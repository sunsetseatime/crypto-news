...
- Paper Trading: dashboard + report now include a clear "Recent activity (last 14 days)" section.
- Signal Engine: missing fundamentals now show as "not tracked" (instead of confusing n/a) when a data source is not available.

## Docs map (so you donâ€™t have to search)
...
Optional: host the dashboard on Vercel (with an embedded chat panel) â€” see `readme/README.md`.
